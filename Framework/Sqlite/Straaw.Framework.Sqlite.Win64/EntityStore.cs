...
// TODO Namespacet System.Data.SQLite; hittas inte i Xamarin Studio.
// Bör fixas när vi vill använda windows phone
...
namespace Straaw.Framework.Sqlite
{
	...
	public class EntityStore : IEntityStore
	{
		...
		{
			using (Log.Scope(LogLevel.Verbose, "GetEntity"))
			{
				using (var connection = new SQLiteConnection(_connectionString))
				{
					connection.Open();
		...
		{
			using (Log.Scope(LogLevel.Verbose, "GetAllEntities"))
			{
				using (var connection = new SQLiteConnection(_connectionString))
				{
					connection.Open();
		...
		{
			using (Log.Scope(LogLevel.Verbose, "PutEntity"))
			{
				bool worked = false;
